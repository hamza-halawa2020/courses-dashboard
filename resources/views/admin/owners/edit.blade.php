...
@section('content')
    ...
    <ul class="breadcrumb mt-2">
        ...
        <li class="breadcrumb-item"><a href="{{ route('admin.owners.index') }}">@lang('owners.owners')</a></li>
                    ...
                    <div class="form-group">
                        ...
                        <select name="role_id" class="form-control select2" required>
                            ...
                            @foreach ($roles as $role)
                                <option value="{{ $role->id }}" {{ $owner->hasRole($role->name) ? 'selected' : '' }}>{{ $role->name }}</option>
                            @endforeach
                        </select>
                    </div>--}}
